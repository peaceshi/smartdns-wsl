...
# Name:        gfwlist2smartdns.sh
# Desription:  A shell script which convert gfwlist into smartdns rules.
# Change From: https://github.com/cokebar
# Author:      peaceshi
...
Author:      peaceshi
Change From: https://github.com/cokebar
Usage: sh gfwlist2smartdns.sh [options] -o FILE
Valid options are:
    -d, --dns <dns_ip>
                DNS IP address for the GfwList Domains (Default: not_cn)
    -p, --port <dns_port>
                DNS Port for the GfwList Domains (Default: )
    -s, --ipset <ipset_name>
    ...
    -l, --domain-list
                Convert Gfwlist into domain list instead of smartdns rules
...
process(){
    ...
    TMP_DIR=`mktemp -d /tmp/gfwlist2smartdns.XXXXXXXX`
    ...
    # Convert domains into smartdns rules
        if [ $WITH_IPSET -eq 1 ]; then
            ...
            sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#nameserver /.\1/'$DNS_IP'\#'$DNS_PORT'\
ipset=/\1/'$IPSET_NAME'#g' > $CONF_TMP_FILE
        else
            _green 'Ipset rules not included.'
            sort -u $DOMAIN_FILE | $SED_ERES 's#(.+)#nameserver /.\1/'$DNS_IP'\#'$DNS_PORT'#g' > $CONF_TMP_FILE
        fi
    ...
    printf '\nRun reload.bat to use new rules.'&& _green 'Finished\n\n'
    # Clean up
    clean_and_exit 0
}
